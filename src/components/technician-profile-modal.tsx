...
export function TechnicianProfileModal({ open, onOpenChange, technician }: TechnicianProfileModalProps) {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        setServices(data || []);
        // Contar servicios activos (status = 'active' o 'in_progress')
        const activeCount = data?.filter(service => 
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-muted-foreground">{technician.address || 'No especificada'}</p>
              </div>
              <div>
                <h3 className="font-semibold">Fecha de registro:</h3>
                <p className="text-muted-foreground">{formatDate(technician.created_at)}</p>
              </div>
              <div>
